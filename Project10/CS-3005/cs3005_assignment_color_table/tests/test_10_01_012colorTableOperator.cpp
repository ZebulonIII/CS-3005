...
TEST_F( ColorTableOperatorTest, changesMutableColorDoesNotChangeOtherColors ) {
  ...
  /* Comparison */
  EXPECT_EQ( expected_changed_color, color_table[ 2 ] ) << " The color at index = 2 should have changed.";
  for( i = 0 ; i < expected_num_color ; i ++ ) {
    if( i != 2 ){
      EXPECT_EQ( expected_unchanged_color, color_table[ i ] ) << " The color not at index = 2 should not have changed.";
    }
  }
...
TEST_F( ColorTableOperatorTest, fetchesConstantErrorColorIndexTooLowVeryLow ) {
  ...
  /* Stimulus */
  const ColorTable& color_table_const = color_table;
  const Color& actual_color = color_table_const[ -1000000 ];
...
TEST_F( ColorTableOperatorTest, fetchesConstantErrorColorIndexTooHighVeryHigh ) {
  ...
  /* Stimulus */
  const ColorTable& color_table_const = color_table;
  const Color& actual_color = color_table_const[ expected_num_color+1000000 ];
...
TEST_F( ColorTableOperatorTest, fetchesMutableErrorColorIndexTooLowVeryLow ) {

  /* Setup */
  // Done automatically

  /* Stimulus */
  Color& actual_color = color_table[ -1000000 ];
...
TEST_F( ColorTableOperatorTest, fetchesMutableErrorColorIndexTooHighVeryHigh ) {

  /* Setup */
  // Done automatically

  /* Stimulus */
  Color& actual_color = color_table[ expected_num_color+1000000 ];
...
TEST_F( ColorTableOperatorTest, fixesMutableErrorColor ) {

  /* Setup */
  // Done automatically

  /* Stimulus */
  Color& tmp_color = color_table[ expected_num_color+1000000 ];
  tmp_color.setRed( 1317 );
  Color& actual_color = color_table[ -1000000 ];

  /* Comparison */
  EXPECT_EQ( expected_error_color, actual_color )
    << "Make sure that you reset the error color to -1,-1,-1 in the mutable (non-const) indexing operator, every time the error occurs.";
